# Organizer Guides

Detailed guides for organizing, branding, sponsoring, and maintaining your event.

## Quick Navigation

- **[Event Organizer Onboarding](event-organizer-onboarding.md)** — Role definitions and readiness checkpoints
- **[Branding & Identity](branding.md)** — Naming, logos, and co-branding guidelines
- **[Sponsorship](sponsorship.md)** — Sponsorship tiers, recruitment, and agreements
- **[Code of Conduct](code-of-conduct.md)** — Expected behavior and reporting procedures
- **[Localization](localization.md)** — Translating content for global audiences

## Key Topics

### Getting Your Team Ready
Learn team roles, responsibilities, and readiness checkpoints:
- Primary Organizer
- Speakers and Lab Facilitators
- Volunteers and Support Staff
- 4-phase readiness planning

### Branding Your Event
Maintain consistency while adding local flavor:
- Naming convention: "Global Security Bootcamp — [City] [Year]"
- Logo usage and color palette
- Co-branding with sponsors
- Social media and website guidelines

### Building Sponsor Relationships
Secure funding and partnerships:
- 3 sponsorship tiers (Supporter, Partner, Principal)
- Sponsorship agreement template
- Recruitment strategies
- Communication timeline

### Setting Community Standards
Create an inclusive, welcoming environment:
- Expected behaviors and conduct
- Reporting procedures
- Confidentiality and enforcement
- Participant and organizer responsibilities

### Going Global
Reach international audiences:
- Translation workflow
- Language folder structure
- Terminology guidelines
- Contribution process for translators

---

**Next Steps**: Start with [Event Organizer Onboarding](event-organizer-onboarding.md) or [Branding & Identity](branding.md)
